...
export const applyLeave = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const hrEmployees = await Employee.find({ role: { $in: ['hr', 'admin'] } });
...
export const approveLeave = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const leave = await Leave.findById(id).populate('employeeId', 'fullName userId');
    if (!leave) {
    ...
    await Notification.create({
      userId: (leave.employeeId as any).userId,
...
export const cancelLeave = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const leave = await Leave.findOne({ _id: id, employeeId: employee._id });
    if (!leave) {
      return res.status(404).json({ message: 'Leave request not found' });
    }
...
export const getLeaveBalance = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const usedLeaves = approvedLeaves.reduce((acc: any, leave) => {
    ...
    const leaveBalance = Object.keys(leaveAllocations).reduce((acc: any, type) => {
      const allocated = leaveAllocations[type as keyof typeof leaveAllocations];
...
export const getLeaveStats = async (req: Request, res: Response) => {
  try {
    ...
    stats.forEach(stat => {
      ...
      formattedStats.byType[stat._id.type as keyof typeof formattedStats.byType] += stat.count;
